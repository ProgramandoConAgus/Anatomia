...
<ul class="nav nav-pills nav-pills-custom mb-3" role="tablist">
    <?php
    // Consultar las categorías dinámicamente
    $sql = "
        SELECT 
            c.IdCategoria, 
            c.titulo AS categoria_titulo 
        FROM 
            categoria_pdfs c where idModulo=".$numero_recibido."
    ...
    ?>
            ...
            <li class="nav-item mb-3 me-3 me-lg-6" role="presentation">
                ...
                <a class="nav-link d-flex justify-content-between flex-column flex-center overflow-hidden w-80px h-85px py-4" 
                   data-bs-toggle="pill" 
                   href="#categoria_<?php echo htmlspecialchars($row['IdCategoria']); ?>" 
                   aria-selected="false" 
                   role="tab" 
                   tabindex="-1">
                    <!--begin::Icon-->
                    ...
                    <span class="nav-text text-gray-700 fw-bold fs-6 lh-1">
                        <?php echo htmlspecialchars($row['categoria_titulo']); ?>
                    </span>
                    <!--end::Subtitle-->
                    <!--begin::Bullet-->
...
<div class="tab-content">
    <?php
    // Consultar las categorías nuevamente para generar los contenedores dinámicos
    $result = $conex->query($sql);
    if ($result && $result->num_rows > 0):
        while ($row = $result->fetch_assoc()):
    ...
    ?>
        <!--begin::Tab Pane-->
<div class="tab-pane fade" id="categoria_<?php echo htmlspecialchars($categoriaId); ?>" role="tabpanel">
    <div class="table-responsive">
        <table class="table align-middle table-row-dashed fs-6 gy-5">
            <tbody>
                <?php
                // Consultar los PDFs asociados a la categoría actual
                $pdfQuery = "
                    SELECT 
					p.Idpdf, 
					p.titulo AS pdf_titulo, 
					p.pdf_path AS pdf_ruta,
					c.descripcion AS categoria_descripcion
				FROM 
					pdfs p
				JOIN 
					categoria_pdfs c ON p.IdCategoria = c.IdCategoria
				WHERE 
					p.IdCategoria = ? 
				

                ";
                $stmt = $conex->prepare($pdfQuery);
                $stmt->bind_param('i', $categoriaId);
                $stmt->execute();
                $pdfResult = $stmt->get_result();

                // Verificar si hay PDFs para mostrar
                if ($pdfResult && $pdfResult->num_rows > 0):
                    while ($pdf = $pdfResult->fetch_assoc()):
                ?>
                    <!--begin::PDF Item-->
                    <tr>
                        <td>
                            <a class="text-gray-800 fw-bold text-hover-primary mb-1 fs-6"><?php echo htmlspecialchars($pdf['pdf_titulo']); ?></a>
                        </td>
                        <td class="text-end">
                            <span class="badge badge-light-success fs-7 fw-bold"><?php echo htmlspecialchars($pdf['categoria_descripcion']); ?></span>
                        </td>
                        <td class="text-end">
                            <span class="text-gray-800 fw-bold d-block fs-6"><a href="<?=$pdf['pdf_ruta']?>" download="<?=$pdf['pdf_titulo'] ?>">Descargar PDF</a></span>
							<?php 
							if($_SESSION['IdTipoUsuario'] == 2){ 
								?>
							<button class="btn btn-danger er fs-6 px-5 py-3" data-toggle="modal" data-target="#confirmDeleteModal<?=$pdf['Idpdf']?>">Eliminar</button>
						<?php 
						}
						?>
                        </td>
					...
					<!-- Modal de Confirmación de Eliminación -->
													<div class="modal fade" id="confirmDeleteModal<?=$pdf['Idpdf']?>" tabindex="-1" role="dialog" aria-labelledby="confirmDeleteLabel<?=$pdf['Idpdf']?>" aria-hidden="true">
														<div class="modal-dialog modal-dialog-centered" role="document">
															<div class="modal-content">
																<div class="modal-header">
																	<h5 class="modal-title" id="confirmDeleteLabel<?=$pdf['Idpdf']?>">Confirmar Eliminación</h5>
																...
																<div class="modal-footer">
																	...
																	<a href="Admins/borrarPdf.php?id=<?=$pdf['Idpdf']?>" class="btn btn-danger">Eliminar</a>
																</div>
															</div>
														</div>
													</div>
                ...
                ?>
                    <tr>
                        <td colspan="3">No hay PDFs disponibles para esta categoría.</td>
                    </tr>
                <?php
                endif;
                ?>

...
if (isset($_GET['token'])) {
    ...
    $stmt = $conex->prepare("SELECT * FROM usuariostemporales WHERE token = ?");
    ...
    if ($result->num_rows > 0) {
        ...
        // Mover a tabla principal
        $stmt_insert = $conex->prepare("INSERT INTO usuarios 
                                      (nombre, apellido, email, password, tipoUsuario, habilitado) 
                                      VALUES (?, ?, ?, ?, ?, ?)");
        $tipoUsuario = 1;
        $habilitado = 2;
        $stmt_insert->bind_param('ssssii', 
                               strtolower(trim($user['nombre'])), 
                               strtolower(trim($user['apellido'])), 
        ...
        if ($stmt_insert->execute()) {
            ...
            $conex->query("DELETE FROM usuariostemporales WHERE email = '" . $user['email'] . "'");
            $message= "Cuenta verificada exitosamente! <a href='index.php'>Iniciar sesión</a>";
        } else {
            $message= "Error al activar la cuenta: " . $conex->error;
        }
    } else {
				...
				<div class="d-flex flex-column flex-center text-center p-10">
					...
					<div class="card card-flush w-md-650px py-5">
						<div class="card-body py-15 py-lg-20">
							...
							<!--begin::Link-->
							<div class="mb-0">
								<a href="index.php" class="btn btn-sm btn-primary">Iniciar Sesion</a>
							</div>
							<!--end::Link-->
						</div>
					</div>
